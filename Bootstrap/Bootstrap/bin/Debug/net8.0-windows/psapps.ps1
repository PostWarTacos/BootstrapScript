...
function Update-AllApps {
    Write-Host "Checking for updates to installed applications..." -ForegroundColor Cyan

    $lines = winget upgrade

    # Find the header line (starts with 'Name')
    $headerIndex = $lines | Select-String -Pattern '^Name' | Select-Object -First 1 | ForEach-Object { $_.LineNumber - 1 }
    if ($headerIndex -eq $null) {
        Write-Host "No header found."
        return
    }

    # Get the header and data lines
    $header = $lines[$headerIndex]
    $dataLines = $lines[($headerIndex + 2)..($lines.Count - 1)] # +2 skips the dashed line

    # Split header into column names
    $columns = $header -split '\s{2,}|¦'
    $columns = $columns | ForEach-Object { $_.Trim() }

    # For each data line, create a PSCustomObject
    $objects = foreach ($line in $dataLines) {
        if ($line.Trim() -eq "") { continue }
        $values = $line -split '\s{2,}|¦'
        $values = $values | ForEach-Object { $_.Trim() }
        if ($values.Count -ne $columns.Count) { continue }
        $props = @{}
        for ($i = 0; $i -lt $columns.Count; $i++) {
            $props[$columns[$i]] = $values[$i]
        }
    ...
    $objects | Format-Table Name, Version, Available -AutoSize

    pause  # Wait for user to review before proceeding

    Write-Host "`nUpgrading all available apps..." -ForegroundColor Yellow
    winget upgrade --all --silent --accept-package-agreements --force --wait
}
